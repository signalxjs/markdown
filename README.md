...
| `@sigx/markdown/dom` | `MarkdownView` for the web — default components styled through `data-scope` / `data-part` attributes, optional Shiki highlighting via `@sigx/markdown/shiki` |
...
| `@sigx/markdown/editor/dom` | `MarkdownEditor` for the web — a Notion-class block editor with toolbar, block menu, slash commands and mentions |
...
export const Answer = component(() => () => <MarkdownView value={stream.value.value} />);
```
