...
export function createSchema(specs: readonly NodeSpec[], options: CreateSchemaOptions = {}): Schema {
    ...
    for (const spec of specs) {
        if (__DEV__ && map.has(spec.type)) console.warn(`[@sigx/richtext] Node spec "${spec.type}" replaces an earlier one.`);
    ...
    const createBlock = (type: string): BlockContent => {
        ...
        throw new Error(`[@sigx/richtext] Cannot create a "${type}" block: no fromInline or menu entry.`);
    };
    return {
        specs: map,
