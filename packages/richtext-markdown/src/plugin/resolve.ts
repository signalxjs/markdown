...
export function resolveMarkdownPlugins(plugins?: readonly RichTextPlugin[] | null): ResolvedMarkdownPlugins {
    ...
    for (const plugin of plugins) {
        if (!plugin || typeof plugin.name !== 'string' || plugin.name === '') {
            if (__DEV__) console.warn('[@sigx/richtext-markdown] Ignoring a plugin without a name.');
            continue;
        }
        if (names.has(plugin.name)) {
            if (__DEV__) console.warn(`[@sigx/richtext-markdown] Duplicate plugin "${plugin.name}" ignored.`);
            continue;
        }
...
function validExtension(
    ...
    if (typeof ext.name !== 'string' || ext.name === '') {
        if (__DEV__) console.warn(`[@sigx/richtext-markdown] Plugin "${pluginName}" registers an extension without a name; ignored.`);
        return false;
    }
    ...
    if (!Array.isArray(chars) || chars.length === 0 || chars.some((c) => typeof c !== 'string' || c.length !== 1)) {
        if (__DEV__) {
            console.warn(
                `[@sigx/richtext-markdown] Extension "${ext.name}" (plugin "${pluginName}") needs a non-empty triggerChars list of single characters; ignored.`,
            );
        }
        return false;
    }
    if (seen.has(ext.name)) {
        if (__DEV__) console.warn(`[@sigx/richtext-markdown] Duplicate extension name "${ext.name}" (plugin "${pluginName}"); ignored.`);
        return false;
    }
    seen.add(ext.name);
    return true;
