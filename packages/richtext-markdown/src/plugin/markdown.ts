/**
 * The markdown slot of the plugin contract — what a plugin contributes to the
 * markdown parser and serializer (`formats.markdown`).
 *
...
import type { BlockContent, Node, Parent, PhrasingContent, Position, Root } from '@sigx/richtext';
...
/** What a plugin contributes to markdown: `formats: { markdown: { … } }`. */
export interface MarkdownPluginSlice {
    ...
    /** Serializer rules keyed by node type (a rule for a built-in type overrides it). */
    // oxlint-disable-next-line no-explicit-any
     ...
     * in dev.
     */
    transformDocument?: (root: Root) => void;
