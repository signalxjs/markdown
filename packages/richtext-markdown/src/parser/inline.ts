...
import type { Image, ImageReference, Link, LinkReference, PhrasingContent, Point, Position, ReferenceType, Text } from '@sigx/richtext';
import type { InlineMatchContext, InlineSyntaxExtension } from '../plugin/markdown.js';
import type { ResolvedMarkdownPlugins } from '../plugin/resolve.js';
import { NO_MARKDOWN_PLUGINS } from '../plugin/resolve.js';
import { decodeEntity, isEscapable, isUnicodePunctuation, isUnicodeWhitespace, normalizeLabel } from '@sigx/richtext';
...
function matchExtension(
    ...
    for (const ext of extensions) {
        ...
        try {
            ...
            if (__DEV__) console.warn(`[@sigx/richtext-markdown] Inline extension "${ext.name}" threw; treated as no match.`, err);
            m = null;
        }
        if (m && m.node && typeof m.node === 'object' && m.end > pos && m.end <= text.length) return m;
        if (__DEV__ && m) {
            console.warn(`[@sigx/richtext-markdown] Inline extension "${ext.name}" returned an invalid end (${m.end}); ignored.`);
        }
    }
    return null;
}
