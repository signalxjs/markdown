...
import type { AlignType, BlockContent, Code, Definition, Heading, HeadingDepth, ListItem, PhrasingContent, Point, Position, Root, Table, TableCell, TableRow } from '@sigx/richtext';
import { childKey, topKey } from '@sigx/richtext';
import type { BlockFinishContext, BlockState, BlockSyntaxExtension, LineInfo } from '../plugin/markdown.js';
import type { ResolvedMarkdownPlugins } from '../plugin/resolve.js';
import { NO_MARKDOWN_PLUGINS } from '../plugin/resolve.js';
import { parseInline } from './inline.js';
import { isSpaceOrTab, matchTableDelimiter, scanDefinition, splitTableRowWithOffsets, unescapeString } from './scanner.js';
...
class BlockParser {
    ...
    private finalizeExtension(block: Block): void {
        ...
        try {
            ...
            if (__DEV__) console.warn(`[@sigx/richtext-markdown] Block extension "${ext.name}" threw in finish(); emitting its lines as a paragraph.`, err);
        ...
        switch (container.type) {
            ...
            case 'extension': {
                ...
                try {
                    ...
                    if (__DEV__) console.warn(`[@sigx/richtext-markdown] Block extension "${ext.name}" threw in continue(); closing the block.`, err);
                    r = 'close';
                }
        ...
        if (maybeExt && !this.indented) {
            ...
            for (const ext of this.plugins.block) {
                ...
                try {
                    ...
                    if (__DEV__) console.warn(`[@sigx/richtext-markdown] Block extension "${ext.name}" threw in start(); ignored.`, err);
                    started = null;
                }
                if (!started) continue;
                this.closeUnmatchedBlocks();
