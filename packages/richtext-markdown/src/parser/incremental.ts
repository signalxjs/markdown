...
import type { BlockContent } from '@sigx/richtext';
import { createLineEngine, type IncrementalEngine } from '@sigx/richtext';
import type { RichTextPlugin } from '@sigx/richtext';
import type { ResolvedMarkdownPlugins } from '../plugin/resolve.js';
import { resolveMarkdownPlugins } from '../plugin/resolve.js';
import { parseBlocks } from './blocks.js';
import { parseInline } from './inline.js';
import { normalizeSource } from './scanner.js';

export type { IncrementalEngine } from '@sigx/richtext';
...
export function createIncrementalEngine(options?: IncrementalEngineOptions): IncrementalEngine {
    ...
    if (__DEV__ && plugins.transformDocument.length) {
        console.warn('[@sigx/richtext-markdown] createIncrementalEngine ignores plugin transformDocument hooks; use parseMarkdown for those.');
    }
    ...
    const transform = (block: BlockContent): BlockContent => {
        ...
        for (const t of plugins.transformBlock) {
            try {
                ...
                if (__DEV__) console.warn('[@sigx/richtext-markdown] transformBlock threw; block left unchanged.', err);
            }
        }
        return node;
    };
