/**
 * `<MarkdownView>` on happy-dom: mounted through `render` from
 * `@sigx/runtime-dom` (tests may use the `sigx` umbrella for JSX).
 */
import { afterEach, describe, expect, it, vi } from 'vitest';
import { component, jsx, signal } from 'sigx';
import { render } from '@sigx/runtime-dom';
import { MarkdownView, createDomComponents } from '@sigx/markdown/dom';
import { mentionPlugin, type MarkdownPlugin } from '@sigx/markdown';
import type { DomMarkdownComponents } from '@sigx/markdown/dom';
...
describe('MarkdownView (default components)', () => {
    it('renders blocks and inline nodes with data-scope / data-part attributes and no classes', () => {
        const c = mount(jsx(MarkdownView, { value: '# Hi\n\nSome **bold** `code` and [a link](https://x.com "T").\n\n- one\n- [x] done\n\n---\n\n> q\n\n| a | b |\n| :-- | --: |\n| 1 | 2 |\n\n```ts\nconst x = 1;\n```' }));
    ...
    it('renders html nodes and unresolved references as literal text', () => {
        const c = mount(jsx(MarkdownView, { value: '<script>alert(1)</script>\n\n[foo][bar]' }));
    ...
    it('resolves references against definitions and sanitises urls', () => {
        const c = mount(jsx(MarkdownView, { value: '[foo][bar] [x](javascript:alert(1))\n\n[bar]: /url "t"' }));
    ...
    it('adds prefixed classes and forwards host attributes to the root', () => {
        const c = mount(jsx(MarkdownView, { value: '# T', classPrefix: 'md', class: 'prose', id: 'doc', 'aria-label': 'Answer' }));
        const root = c.firstElementChild as HTMLElement;
        ...
        const c2 = mount(jsx(MarkdownView, { value: '```ts\nx\n```', classPrefix: 'md' }));
    ...
    it('routes link clicks to onLink with the event and prevents navigation', () => {
        const onLink = vi.fn();
        const c = mount(jsx(MarkdownView, { value: '[go](https://example.com)', onLink }));
    ...
    it('sets linkTarget on external links only', () => {
        const c = mount(jsx(MarkdownView, { value: '[a](https://x.com) [b](/local)', linkTarget: '_blank' }));
        const links = c.querySelectorAll('a');
    ...
    it('accepts component overrides and plugin node components', () => {
        const components: Partial<DomMarkdownComponents> = {
        ...
        const c = mount(jsx(MarkdownView, { value: '# T\n\nhi @[Andy](u1)', components, plugins }));
    ...
    it('renders a plugin node without a component through its serialize rule as text', () => {
        const c = mount(jsx(MarkdownView, { value: 'hi @[Andy](u1)', plugins: [mentionPlugin] }));
        expect(c.textContent).toBe('hi @[Andy](u1)');
    });

    it('renders a pre-parsed root', () => {
        const root = { type: 'root', children: [{ type: 'paragraph', children: [{ type: 'text', value: 'pre-parsed' }] }] } as never;
        const c = mount(jsx(MarkdownView, { root }));
...
describe('MarkdownView (streaming)', () => {
    ...
    it('re-creates the engine when the plugins prop changes identity', async () => {
        ...
        const App = component(() => () => jsx(MarkdownView, { value: 'hi @[Andy](u1)', plugins: plugins.list }));
        const c = mount(jsx(App, {}));
...
describe('CodeBlock copy button', () => {
    it('copies the code and shows a transient copied state', async () => {
        ...
        const c = mount(jsx(MarkdownView, { value: '```\nabc\n```' }));
    ...
    it('omits the button when the Clipboard API is absent or copyButton is false', () => {
        ...
        expect(mount(jsx(MarkdownView, { value: '```\nabc\n```' })).querySelector('button')).toBeNull();
