 ...
 *
 * Descends into containers (and into types the schema does not know, so a
 * plugin block's children are keyed) but never into a known non-container
 * block: a text block's phrasing content stays unkeyed even when some of its
 * types are unknown to the schema.
 */
...
function assignBlockKeys(node: MarkdownNode, key: string, schema: Schema): void {
    ...
    if (!children) return;
    if (schema.get(node.type) && !schema.isContainer(node.type)) return;
