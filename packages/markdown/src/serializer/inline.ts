...
import type { SerializeContext, SerializeRule } from '../plugin/index.js';
...
export function runRule(rule: SerializeRule, node: Node, state: State): string {
    try {
        ...
        if (__DEV__) console.warn(`[@sigx/markdown] Serialize rule for "${node.type}" returned a non-string; ignored.`);
    } catch (error) {
        if (__DEV__) console.warn(`[@sigx/markdown] Serialize rule for "${node.type}" threw; node skipped.`, error);
    }
    return '';
}
...
export function serializeUnknown(node: Node, state: State): string {
    if (__DEV__) console.warn(`[@sigx/markdown] No serialize rule for node type "${node.type}"; serializing its children.`);
