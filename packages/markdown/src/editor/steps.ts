...
import { buildIndex, isContainerType } from './state.js';

export type Step =
    /** Replace `[from, to)` of an inline container's flat text with `slice`. */
    | { type: 'replaceInline'; key: string; from: number; to: number; slice: InlineFlat }
    /** Replace an inline container's whole content (IME commit, surface read-back). */
...
export function updateBlock(root: Root, key: string, fn: (node: EditorBlock) => EditorBlock): Root {
    const index = buildIndex(root, new Set());
...
export function updateChildren(root: Root, parentKey: string | null, fn: (children: EditorBlock[]) => EditorBlock[]): Root {
    if (parentKey === null) return { ...root, children: fn(root.children as EditorBlock[]) as BlockContent[] };
    return updateBlock(root, parentKey, (node) => {
        if (!isContainerType(node.type)) throw new StepError(`Block "${parentKey}" (${node.type}) cannot contain blocks.`);
...
export function rekey(node: EditorBlock, key: string): EditorBlock {
    if (!isContainerType(node.type)) return node.key === key ? node : { ...node, key };
    const children = ((node as { children: EditorBlock[] }).children ?? []).map((c, i) => rekey(c, childKey(key, i)));
    return { ...node, key, children } as EditorBlock;
}

/** Re-key the children of a container so they are `<parentKey>.<i>` (root children: `b-<i>`). */
export function rekeyChildren(children: EditorBlock[], parentKey: string | null): EditorBlock[] {
    return children.map((c, i) => rekey(c, parentKey === null ? `b-${i}` : childKey(parentKey, i)));
}
...
function inlineChildren(node: EditorBlock): PhrasingContent[] {
    const children = (node as { children?: unknown }).children;
    if (!Array.isArray(children) || isContainerType(node.type)) {
        throw new StepError(`Block "${node.key}" (${node.type}) has no inline content.`);
    }
    return children as PhrasingContent[];
}

/** The flat model of an inline container. */
export function flatOf(node: EditorBlock, ctx?: StepContext): InlineFlat {
    return toFlat(inlineChildren(node), ctx?.inline);
}
...
export function applyStep(root: Root, step: Step, ctx: StepContext = {}): Root {
    switch (step.type) {
        case 'replaceInline':
            return updateBlock(root, step.key, (node) => {
                ...
                const next = spliceFlat(flat, step.from, step.to, step.slice);
                return { ...node, children: toInline(next, ctx.inline) } as EditorBlock;
            });
        case 'setInline':
            return updateBlock(root, step.key, (node) => {
                inlineChildren(node);
                return { ...node, children: toInline(step.flat, ctx.inline) } as EditorBlock;
            });
        case 'setValue':
            return updateBlock(root, step.key, (node) => {
        ...
        case 'replaceBlock':
            return updateBlock(root, step.key, () => rekey(step.node, step.key));
        case 'insertBlock':
            return updateChildren(root, step.parentKey, (children) => {
                ...
                return rekeyChildren(next, step.parentKey);
            });
        case 'removeBlock':
            return updateChildren(root, step.parentKey, (children) => {
        ...
        case 'moveBlock':
            return applyMove(root, step).root;
        case 'replaceDoc':
            return step.doc;
    }
}
 ...
 */
export function applyMove(root: Root, step: Extract<Step, { type: 'moveBlock' }>): { root: Root; movedKey: string } {
    const index = buildIndex(root, new Set());
    ...
    if (step.to.parentKey !== null && !isContainerType(dest.type)) {
    ...
    const rebuild = (parent: EditorParent, parentKey: string | null): EditorParent => {
        ...
        if (!isSource && !isDest && !containsAny(parent, [entry.parent, dest])) {
            return parentKey === null || (parent as EditorBlock).key === parentKey ? parent : rekey(parent as EditorBlock, parentKey);
        }
        ...
        next = next.map((c, i) => {
            ...
            return isContainerType(c.type) ? (rebuild(c, k) as EditorBlock) : rekey(c, k);
        });
...
function containsAny(parent: EditorParent, targets: EditorParent[]): boolean {
    const children = (parent as { children?: EditorBlock[] }).children;
    if (!children) return false;
    for (const c of children) {
        if (targets.includes(c)) return true;
        if (isContainerType(c.type) && containsAny(c, targets)) return true;
    }
    return false;
}
...
export function invertStep(root: Root, step: Step, ctx: StepContext = {}): Step {
    switch (step.type) {
        case 'replaceInline': {
            const node = getBlock(root, step.key);
            const flat = flatOf(node, ctx);
        ...
        case 'setInline':
            return { type: 'setInline', key: step.key, flat: flatOf(getBlock(root, step.key), ctx) };
        case 'setValue':
            return { type: 'setValue', key: step.key, value: (getBlock(root, step.key) as { value: string }).value };
        case 'setAttrs': {
            const node = getBlock(root, step.key) as unknown as Record<string, unknown>;
            const attrs: Record<string, unknown> = {};
            for (const k of Object.keys(step.attrs)) attrs[k] = node[k];
            return { type: 'setAttrs', key: step.key, attrs };
        }
        case 'replaceBlock':
            return { type: 'replaceBlock', key: step.key, node: getBlock(root, step.key) };
        case 'insertBlock':
            return { type: 'removeBlock', parentKey: step.parentKey, index: step.index };
        case 'removeBlock': {
            const children = childrenOf(root, step.parentKey);
            return { type: 'insertBlock', parentKey: step.parentKey, index: step.index, node: children[step.index] };
        }
        case 'moveBlock': {
            const entry = buildIndex(root, new Set()).get(step.key);
            if (!entry) throw new StepError(`Unknown block key "${step.key}".`);
            ...
            const { root: after, movedKey } = applyMove(root, step);
            const originalParentKey = entry.parentKey === null ? null : (findKey(after, entry.parent as EditorBlock) ?? entry.parentKey);
 ...
 */
function findKey(root: Root, node: EditorBlock): string | undefined {
    const oldChildren = (node as { children?: EditorBlock[] }).children ?? [];
    const index = buildIndex(root, new Set());
    for (const k of index.keys()) {
        const n = index.get(k)!.node;
        if (n === node) return k;
        if (n.type !== node.type || !isContainerType(n.type)) continue;
...
export function getBlock(root: Root, key: string): EditorBlock {
    const entry = buildIndex(root, new Set()).get(key);
    if (!entry) throw new StepError(`Unknown block key "${key}".`);
    return entry.node;
}

function childrenOf(root: Root, parentKey: string | null): EditorBlock[] {
    if (parentKey === null) return root.children as EditorBlock[];
    const node = getBlock(root, parentKey);
